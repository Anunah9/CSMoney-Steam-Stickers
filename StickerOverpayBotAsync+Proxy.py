...
async def create_async_session(steamclient, addr: str) -> aiohttp.ClientSession:
...
async def get_active_async_sessions(ips):
    sessions = []
    for ip in ips:
        session = await create_async_session(steamclient=params.steamAccMain.steamclient, addr=ip)
        try:
            response1 = await session.get(
                'https://steamcommunity.com/market/listings/730/AK-47%20%7C%20Elite%20Build%20(Minimal%20Wear)')
        except aiohttp.client_exceptions.ClientConnectorError:
            continue
        if response1.status == 429:
            continue
        sessions.append({ip: session})
        print(f"Проверка работы {ip}: ", response1.status)
    if not sessions:
        raise NoActiveSessions('No Active Sessions')
    return sessions


async def main():
    ips = get_adapters_ips()
    params.bot_error_logger.send_message(368333609, 'Получаю активные ip адреса.')
    try:
        sessions = await get_active_async_sessions(ips)
    except NoActiveSessions:
        return
    params.bot_error_logger.send_message(368333609, f'Активных ip адресов: {len(sessions)}.')
    start = 0
    ...
    while True:
        ...
        try:
            ...
            for i in range(iter_time):
                for item in items:
                    print('Выбор текущей сессии: ', sessions[(i + items.index(item)) % len(sessions)].values())
                    tasks1.append(fetch_data(sessions[(i + items.index(item)) % len(sessions)].values(), item, counter))
                    counter += 1
            t2 = time.time()
            ...
            print('Количество выполненных запросов: ', params.counter_200_resp)
            params.counter_200_resp = 0
            t3 = time.time()
